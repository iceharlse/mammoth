...
# ============================================================================
@register_model("er_dam_cagradV12")
class ER_DAM_V11_Final(ContinualModel):
    NAME = "er_dam_cagradV12"
    ...
    def _build_ctx(self, loss_new, loss_old, feats_new, feats_old):
        ...
        # [V12 Improvement: DB-MTL Loss Balancing]
        # 使用 Log 变换来压缩 Loss 的动态范围，防止数值主导。
        # log(1 + loss) 是为了保证 loss=0 时输出 0，且非负。
        log_l_n = math.log1p(l_n_val) # log(1 + x)
        log_l_o = math.log1p(l_o_val)
        
        denom = log_l_n + log_l_o + 1e-8
        
        # 构造 Context：使用 Log 后的值
        ctx = torch.tensor([
            log_l_o / denom,            # 0: Normalized Log Old
            log_l_n / denom,            # 1: Normalized Log New
            log_l_n - log_l_o,          # 2: Log Diff (Scale-invariant gap)
            log_l_n + log_l_o,          # 3: Log Sum
            self.current_task_id / 5.0, # 4: Task Progress
            0, 0, 0 
