...
# ============================================================================
# 1. Controller (Same as V12/V14)
# ============================================================================
class AlphaBetaControllerV12(nn.Module):
    def __init__(
        self,
        feature_dim: int,
        d_model: int = 64,
...
# ============================================================================
# 2. ER + DAM V15 (MGDA + Cosine Adaptive Boosting)
# ============================================================================
@register_model("er_dam_cagradV15")
class ER_DAM_V15_Adaptive(ContinualModel):
    NAME = "er_dam_cagradV15"
    ...
    def get_parser(parser: ArgumentParser) -> ArgumentParser:
        ...
        # [NEW] Boost factor for synergistic gradients
        parser.add_argument("--synergy_boost", type=float, default=2.0, 
                            help="Max boost factor when cos > 0")
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        self.controller = AlphaBetaControllerV12(
    ...
    # ----------------------------------------------------------------------
    # Observation Loop (V15: Cosine Adaptive)
    # ----------------------------------------------------------------------
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        alpha_mgda = self._solve_mgda(grads_new, grads_old)
        ...
        for i, (layer_name, layer_params) in enumerate(layer_groups.items()):
            ...
            # 1. Calc Stats (Norms + Dot Prod)
            gn_norm_sq, go_norm_sq, dot_prod = 0.0, 0.0, 0.0
            
            # First pass to compute stats
            for p in layer_params:
                gn = p_to_gn.get(p)
                go = p_to_go.get(p)
                if gn is not None: 
                    gn_flat = gn.view(-1)
                    gn_norm_sq += torch.dot(gn_flat, gn_flat).item()
                if go is not None: 
                    go_flat = go.view(-1)
                    go_norm_sq += torch.dot(go_flat, go_flat).item()
                if gn is not None and go is not None:
                    dot_prod += torch.dot(gn.view(-1), go.view(-1)).item()
            
            gn_norm = math.sqrt(gn_norm_sq)
            go_norm = math.sqrt(go_norm_sq)
            
            # 2. Adaptive Scaling
            if gn_norm > 1e-8 and go_norm > 1e-8:
                # Base scaler (Conservative)
                base_scaler = go_norm / gn_norm
                
                # Cosine Similarity
                cos_theta = dot_prod / (gn_norm * go_norm + 1e-8)
                
                # [NEW] Adaptive Boost Logic
                # If cos > 0 (Synergy), we allow new gradients to be larger (up to 3x)
                # If cos <= 0 (Conflict), we stick to base_scaler (Safety)
                
                boost_cap = getattr(self.args, "synergy_boost", 2.0)
                
                if cos_theta > 0:
                    # Linear boost: 1.0 at cos=0 -> boost_cap at cos=1
                    boost = 1.0 + cos_theta * (boost_cap - 1.0)
                    scaler = base_scaler * boost
                    
                    # Hard cap at 1.0? 
                    # V5 success suggests we might want > 1.0 sometimes, 
                    # but let's be safe: cap at 1.5 or 2.0 RELATIVE to base scaler?
                    # No, let's cap absolute scaler at 1.0 to prevent explosion,
                    # but closer to 1.0 than V14 allowed.
                    scaler = min(scaler, 1.5) 
                else:
                    # Conflict: Strictly match norms
                    scaler = min(base_scaler, 1.0)
            else:
                scaler = 1.0
            ...
            for p in layer_params:
                ...
                grad_plastic = (w_new_final * beta_l) * (gn * scaler)
