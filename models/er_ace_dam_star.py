...
class SwitchableCosineLinear(nn.Linear):
    """A STAR-compatible version of ER-ACE's cosine classifier.

    Why this exists:
      - STAR's Perturber builds a *proxy* ResNet whose classifier is a standard nn.Linear
        with state_dict keys: `classifier.weight` and `classifier.bias`.
      - ER-ACE's original CustomLinear stores weights under `classifier.L.weight` (no bias),
        which breaks `proxy.load_state_dict(net.state_dict())`.

    This module keeps the **same parameter names** as nn.Linear (weight/bias) so the proxy
    can load, while still allowing cosine-style logits during the main ER-ACE/DAM update.

    We also support a temporary "linear" mode that makes this module behave exactly like
    nn.Linear, so during the STAR perturbation step we can align net outputs with the proxy
    without touching STAR's code.
    """
    ...
    def __init__(
        ...
        self._mode = "cosine"  # "cosine" or "linear"

        # ER-ACE's original cosine head has no bias. We keep a bias tensor only for
        # STAR proxy compatibility, but freeze it at 0.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # cosine mode (ER-ACE style)
        x_n = F.normalize(x, p=2, dim=1, eps=1e-5)
        w_n = F.normalize(self.weight, p=2, dim=1, eps=1e-5)
        return self.scale_factor * (x_n @ w_n.t())


class CustomLinear(torch.nn.Module):
    """Legacy ER-ACE cosine head (kept only for reference).

    NOTE: This module is **not STAR-compatible** because its state_dict keys are
    `classifier.L.weight` and it has no `classifier.bias`.
    """
...
@register_model("er_ace_dam_star")
class ErACEDamAlphaBetaConstrainedV3(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        if args.use_custom_classifier:
            ...
            # Replace with STAR-compatible cosine head that still exposes
            # `classifier.weight`/`classifier.bias`.
            in_f = backbone.classifier.in_features
            out_f = backbone.classifier.out_features

            new_head = SwitchableCosineLinear(
                in_features=in_f,
                out_features=out_f,
                scale_factor=10.0,
                bias=True,
            )
            # Keep deterministic init: start from the original linear head weights.
            with torch.no_grad():
                new_head.weight.copy_(backbone.classifier.weight.data)
                # bias is frozen at 0 by SwitchableCosineLinear
    ...
    # ---------------- STAR compatibility helpers ----------------
    def _star_enter_linear_mode(self):
        """Make net classifier behave like nn.Linear just for STAR perturbation.

        This avoids semantic mismatch between STAR proxy (always linear) and ER-ACE
        cosine head, without modifying STAR's code.

        We also (optionally) freeze classifier weights during the STAR step to avoid
        mixing "linear" classifier updates into the cosine training dynamics.
        """
        self._star_prev = {}
        clf = getattr(self.net, "classifier", None)
        if clf is None:
            return

        if hasattr(clf, "mode") and hasattr(clf, "set_mode"):
            self._star_prev["clf_mode"] = clf.mode
            clf.set_mode("linear")

        # Freeze classifier params during STAR step (safer when main training uses cosine).
        if hasattr(clf, "weight") and isinstance(clf.weight, torch.nn.Parameter):
            self._star_prev["w_req"] = clf.weight.requires_grad
            clf.weight.requires_grad = False
        if hasattr(clf, "bias") and isinstance(getattr(clf, "bias", None), torch.nn.Parameter):
            self._star_prev["b_req"] = clf.bias.requires_grad
            clf.bias.requires_grad = False

    def _star_exit_linear_mode(self):
        clf = getattr(self.net, "classifier", None)
        if clf is None:
            return
        prev = getattr(self, "_star_prev", {})

        if "w_req" in prev and hasattr(clf, "weight"):
            clf.weight.requires_grad = prev["w_req"]
        if "b_req" in prev and getattr(clf, "bias", None) is not None:
            clf.bias.requires_grad = prev["b_req"]
        if "clf_mode" in prev and hasattr(clf, "set_mode"):
            clf.set_mode(prev["clf_mode"])
        self._star_prev = {}
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        # 0) STAR perturbation step (plugin).
        # Run a small perturbation update on a buffer minibatch before computing ER-ACE/DAM losses.
        if not self.buffer.is_empty():
            buf_inputs_star, buf_labels_star = self.buffer.get_data(
                self.args.minibatch_size, transform=self.transform
            )
            # Make classifier temporarily linear so STAR proxy and net match.
            self._star_enter_linear_mode()
            try:
                self.pert(buf_inputs_star, buf_labels_star)
            finally:
                self._star_exit_linear_mode()
