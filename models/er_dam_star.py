...
@register_model("er_dam_star")
class ERDamAlphaBetaSTAR(ContinualModel):
    """
    ...
    def _build_ctx(self, loss_new, loss_old, features_new, features_old):
        ...
        t_norm = self.current_task_id / max((getattr(self.args, "total_tasks", 5) or 5) - 1, 1)
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        # STAR: parameter perturbation using a buffer minibatch (if available)
        if not self.buffer.is_empty():
            p_buf_inputs, p_buf_labels = self.buffer.get_data(
                self.args.minibatch_size, transform=self.transform
            )
            self.pert(p_buf_inputs, p_buf_labels)
